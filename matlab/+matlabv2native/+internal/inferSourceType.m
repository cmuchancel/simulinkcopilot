...
% inferSourceType Determine which shared source type best matches the MATLAB input.
...
if ~any(strcmp(requested, supported))
    error("matlabv2native:InvalidSourceType", ...
...
if isa(primaryInput, "function_handle")
    error("matlabv2native:OpaqueOdeFunction", ...
...
texts = matlabv2native.internal.equationTexts(primaryInput);
if isempty(texts)
    error("matlabv2native:UnsupportedInputType", ...
        "matlabv2native accepts symbolic equations, text equations, LaTeX equations, or structured ODE specs.");
end
